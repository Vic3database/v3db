...
> 已于 2026-08-02 完成。悬停省份代码改为按需采样既有 `provinces.png`，未生成重复的省份颜色游程索引。

**Goal:** 在地域板块中提供受现有地域筛选约束的省份地形视图。

**Architecture:** 构建脚本从游戏的 `province_terrains.txt` 和 `provinces.png` 生成地域、开局所有权和地形三组像素游程索引。地图运行时一次解码地形索引，悬停时按需采样既有省份图片获得省份代码；地形模式以省份层级绘制颜色并保留地域交互。筛选栏入口只切换地域地图模式，右侧地域列表沿用原有渲染。
...
在 `scripts/check_province_terrain_map.mjs` 中执行每份 `map-data.js`，并要求包含 `terrainKeys` 与 `terrainRuns`。断言十种陆地键和 `ocean`、`lakes` 都存在，地形游程解码后覆盖 `width * height` 个像素。
...
读取每个符合 `^x([0-9A-Fa-f]{6})\s*=\s*"([^"]+)"` 的行，规范化为 `x` 加大写六位色码。按地形键建立 `terrainKeys` 与 `$colorToTerrainIndex`，在现有像素循环中写入 `terrainRuns`。省份颜色代码由前端从既有图片按需采样；保留当前地域和开局所有权索引的字段名及行为。
...
断言要求 `mapRuntime` 包含 `terrainKeysByIndex` 与 `pixelTerrainIndexes`；`ensureMapLoaded()` 解码地形游程；`drawMapLayer()` 在 `terrain` 模式按省份像素赋色；水体索引保持透明；`mapTooltipHtml()` 在地形模式输出由源省份图采样得到的代码和地形。
...
- [x] **Step 2: 运行断言并确认失败原因是运行时尚未读取地形索引**

运行：`node scripts/check_province_terrain_map.mjs`

预期：失败信息指出地形运行时字段、绘制分支或提示字段缺失。

- [x] **Step 3: 实现一次性索引解码与清理**

在 `site/app/runtime.js` 的 `mapRuntime` 增加地形索引、源省份图和一像素采样画布。在 `site/app/data.js` 的 `resetMapRuntime()` 中一并复位。在 `ensureMapLoaded()` 中读取：
...
mapRuntime.pixelTerrainIndexes = mapData.terrainRuns
  ...
  : null;
mapRuntime.provinceMapImage = await loadImage(mapRuntime.imageUrl);
```

- [x] **Step 4: 实现地形颜色、按像素渲染和提示字段**
...
- [x] **Step 1: 运行主站和 Victorian Century 的本地服务**
...
- [x] **Step 5: 写入工作记录并提交验证记录**

原工作树已有未跟踪的 `docs/worklog/2026-08-02.md`，为避免覆盖，将验证记录写入 `docs/worklog/2026-08-02-province-terrain-map.md`。记录包括数据来源、12 类源数据与 10 类陆地图例、构建输出、地图筛选规则、浏览器验证尺寸和通过的检查命令。

```powershell
git add docs/worklog/2026-08-02-province-terrain-map.md
