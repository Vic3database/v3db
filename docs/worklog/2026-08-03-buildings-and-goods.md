...
建筑详情每个生产方式组只显示当前选中的图标。点击图标才展开同组的其他图标选项；选择后立即按当前组合重算。组合结果默认按一级建筑显示，将就业简化为“劳动力：3000劳工，1000农民”，投入和产出简化为“投入商品：2工具”“产出商品：20谷物”，其余无条件和条件效果保留在修正栏。生产方式详情收纳在默认关闭的折叠框中。标准产值按商品基础价格计算：产出价值减投入价值后得到周利润，再除以当前劳动力并乘 52，显示为年人均利润；计算方式参照 `D:\下载\产值表1.12.0.xlsx`。油井提供两项基础生产方式和三项自动化生产方式，可逐项组成六种选择结果。农业与资源建筑会自动选中相应资源筛选器，跳转至 `#/region/resource/<building-key>`；资源类型由地区数据判定，小麦农场使用可耕地资源，油井使用上限资源。
