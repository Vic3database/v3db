...
主站 1.13.9 新增“建筑”和“商品”两个独立板块，数据来自 `D:\SteamLibrary\steamapps\common\Victoria 3\game`。建筑墙按建筑组分区，收录 101 个具有原始图标的建筑；14 个 `bg_monuments_hidden` 图形定义没有图标，记录在数据库排除清单中，不生成卡片。商品墙收录 53 种基础商品，详情仅列出可生产建筑；72 种名贵商品作为对应基础商品的详情变体显示。
