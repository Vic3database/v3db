...
建筑详情每个生产方式组只显示当前选中的图标。点击图标才展开同组的其他图标选项；选择后立即按当前组合重算。组合结果固定显示雇用人群、投入商品、产出商品、标准产值和修正，无条件效果按作用域、缩放方式与内部键合并，有条件效果列入修正并标注条件。生产方式详情收纳在默认关闭的折叠框中。标准产值目前保留接口并显示“待接入”，暂不推算数值。油井提供两项基础生产方式和三项自动化生产方式，可逐项组成六种选择结果。农业与资源建筑会自动选中相应资源筛选器，跳转至 `#/region/resource/<building-key>`；资源类型由地区数据判定，小麦农场使用可耕地资源，油井使用上限资源。
