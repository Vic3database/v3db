...
使用唯一入口 `#/culture/incorporation`。直接进入时不绑定国家，候选区提供当前资料库的文化；从国家详情点击入口时，只把该国主流文化和已审计候选预载到侧栏，地址仍保持唯一入口。地图保持主视区，文化选择放在左侧栏；候选文化只修改待计算集合，点击“开始计算”后才应用颜色。页面不再显示独立的“整合结果”板块，地图提示承担结果查看。
