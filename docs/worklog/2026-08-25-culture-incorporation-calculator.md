...
新增 `#/culture/incorporation` 和 `#/culture/incorporation/<TAG>` 路由。进入奥地利时默认选中南德意志文化，并从已审计的联邦方案路径加载匈牙利、捷克、斯洛伐克、克罗地亚、塞尔维亚、斯洛文尼亚、波兰、罗马尼亚、乌克兰、北意大利和塞克利文化候选。地图保持主视区，文化选择放在左侧栏；候选文化只修改待计算集合，点击“开始计算”后才应用颜色和结果。用户可以搜索、添加、删除文化；空集合在启动计算后显示空状态。新增独立 `cultureIncorporation` 地图模式和地域结果列表，复用 2、5、10、15、25 年计算规则。
