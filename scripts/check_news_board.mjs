...
assert(items.some((item) => item.id === "patch-1.13.10"), "news data must contain patch 1.13.10");
assert(items.some((item) => item.id === "patch-1.13.11"), "news data must contain patch 1.13.11");
...
assert.match(app, /t\("nav\.news"\)/, "news view label must use the localized navigation key");
...
assert.match(app, /all: "news\.category\.all"/, "home news must include the all category");
assert.match(app, /diary: "news\.category\.diary"/, "home news must include the diary category");
assert.match(app, /patch: "news\.category\.patch"/, "home news must include the patch category");
assert.match(app, /other: "news\.category\.other"/, "home news must include the other category");
assert.match(app, /data-news-category="\$\{key\}"/, "home news category buttons must expose category values");
assert.match(app, /data-news-more/, "home news must expose the more link");
...
const mobileStyles = styles.match(/@media \(max-aspect-ratio: 3 \/ 2\) \{([\s\S]*?)\n}\n\n@media \(max-width: 520px\)/)?.[1] || "";
