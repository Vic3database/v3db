...
assert.equal(reviews.length, 187, "图片复核记录总数不一致");
assert.equal(personReviews.length, 14, "人物终态总数不一致");
assert.equal(reviews.filter((review) => review.decision === "approve").length, 173, "已批准图片总数不一致");
assert.equal(reviews.filter((review) => review.decision === "reject").length, 14, "已拒绝图片总数不一致");
