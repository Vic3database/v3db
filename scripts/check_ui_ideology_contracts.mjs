...
function checkTypographyContracts() {
  ...
  const genericResultCountryGridIndex = styleSource.search(/\.results \.country-row \{\r?\n  grid-template-columns: 64px minmax\(0, 1fr\) 28px;/);
  const companyResultGridIndex = styleSource.search(/\.results \.company-row \{\r?\n  grid-template-columns: minmax\(0, 1fr\);/);
...
function checkRedesignContracts() {
  ...
  assert(/#heritageGroupFilters\s+\.filter-group-block\s*\+\s*\.filter-group-block,\s*[\s\S]*#languageGroupFilters\s+\.filter-group-block\s*\+\s*\.filter-group-block\s*{[\s\S]*padding-top:\s*10px[\s\S]*border-top:\s*1px solid rgba\(117,\s*99,\s*83,\s*0\.22\)/.test(styleSource), "heritage and language groups should be divided by consistent spacing and separators");
  assert(/#heritageGroupFilters\s+\.filter-row-items,\s*[\s\S]*#languageGroupFilters\s+\.filter-row-items\s*{[\s\S]*gap:\s*6px/.test(styleSource), "heritage and language trait rows should keep readable spacing from their group labels");
