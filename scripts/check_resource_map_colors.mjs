...
assert.match(englishUiSource, /"map\.vcResourceContext": "Victorian Century\/Explorable Real-World Resources"/, "English resource context must use the approved Victorian Century label");
...
assert.match(mapStylesSource, /\.map-resource-context-name\s*\{[\s\S]*overflow-wrap:\s*anywhere[\s\S]*white-space:\s*normal/, "resource context name must wrap instead of truncating when the viewport is narrow");
...
assert.match(indexSource, /styles\.css\?v=20260805-subsistence-gradient1/, "main entry must invalidate changed map styles");
assert.match(stylesEntrySource, /@import url\("styles\/map\.css\?v=20260805-subsistence-gradient1"\);/, "style entry must invalidate the changed map stylesheet");
assert.match(indexSource, /app\/runtime\.js\?v=20260805-subsistence-gradient1/, "main entry must invalidate the changed map runtime");
assert.match(indexSource, /app\/map\.js\?v=20260805-subsistence-gradient1/, "main entry must invalidate the changed map script");
