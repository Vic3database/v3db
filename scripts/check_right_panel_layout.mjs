...
function checkDetailLayoutContracts() {
  ...
  assert(/styles\.css\?v=20260725-tooltip-layout3/.test(readText("site/index.html")), "detail title style changes should refresh the main stylesheet URL");
  assert(/styles\/records\.css\?v=20260725-tooltip-layout3/.test(readText("site/styles.css")), "detail title alignment should refresh the records stylesheet import");
  assert(/styles\/shell\.css\?v=20260725-localized-terms1/.test(readText("site/styles.css")), "ownership category styling should refresh the shell stylesheet import");
  ...
  assert(!/\.culture-detail\s+\.detail-title\s*{[\s\S]*align-items:\s*center/.test(styleSource), "culture detail should not need a one-off vertical alignment override");
}
...
function checkTooltipContracts() {
  ...
  assert(/function\s+conceptTooltipActionHints\s*\(/.test(appSource), "concept tooltip action hints should be isolated and short");
...
function checkListInteractionContracts() {
  ...
  assert(!/data-company-detail|rowDetailButton/.test(functionSource("renderCompanyList")), "company cards should not render a redundant detail-arrow button");
  assert(/row\.addEventListener\("click"[\s\S]*openCompanyDetail\(row\.dataset\.company\)/.test(functionSource("renderCompanyList")), "company card click should open the company detail page");
