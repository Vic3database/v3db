...
const version = process.env.VICTORIA3_VERSION || "1.13.9";
const expectedEvents = version === "1.13.9" ? 2236 : 2239;
const expectedFlavorEvents = version === "1.13.9" ? 836 : 473;
const expectedGenericEvents = expectedEvents - expectedFlavorEvents;
...
try {
  ...
  await page.goto(`${baseUrl}?version=${version}#/event`);
  await page.waitFor(() => document.querySelectorAll("[data-event-id]").length === window.__VICDATA_EXPECTED_EVENTS__, "event list");
  assert.equal(await page.evaluate(() => (document.querySelector("#eventCoverage")?.textContent || "").replaceAll(",", "").match(/\d+/)?.[0] || ""), String(expectedEvents), "event coverage text must match loaded event count");
  ...
  assert.equal(overview.cards, expectedEvents, "event board must show all game events");
  ...
  await page.waitFor(() => document.querySelectorAll("[data-event-id]").length === window.__VICDATA_EXPECTED_FLAVOR_EVENTS__, "flavor event filter");
  ...
  await page.waitFor(() => document.querySelectorAll("[data-event-id]").length === window.__VICDATA_EXPECTED_EVENTS__, "reset before generic event filter");
  ...
  await page.waitFor(() => document.querySelectorAll("[data-event-id]").length === window.__VICDATA_EXPECTED_GENERIC_EVENTS__, "generic event filter");
  ...
  await page.evaluate(() => document.querySelector("#eventResetButton").click());
  await page.waitFor(() => document.querySelectorAll("[data-event-id]").length === window.__VICDATA_EXPECTED_EVENTS__, "reset flavor event filter");
  ...
  await page.evaluate(() => document.querySelector("#eventResetButton").click());
  await page.waitFor(() => document.querySelectorAll("[data-event-id]").length === window.__VICDATA_EXPECTED_EVENTS__, "reset event groups");
