...
assert.match(indexHtml, /app\/culture-incorporation\.js\?v=20260906-calculator-back2/);
...
assert.match(calculator, /clearCultureIncorporationCalculatorState\(\);[\s\S]*state\.detailKind = "culture";[\s\S]*cultureIncorporationPanel\) els\.cultureIncorporationPanel\.hidden = true/);
assert.doesNotMatch(calculator, /function renderCultureIncorporationCalculator\(\)[\s\S]*cultureIncorporationPanel\) els\.cultureIncorporationPanel\.hidden = false/);
assert.match(ui, /cultureIncorporationPanel\.style\.display = cultureCalculator \? "" : "none"/);
assert.match(ui, /if \(!cultureCalculator\) els\.cultureIncorporationPanel\.replaceChildren\(\)/);
assert.match(read("site/styles/records.css"), /\.culture-incorporation-panel\[hidden\]\s*\{[\s\S]*display:\s*none\s*!important/);
