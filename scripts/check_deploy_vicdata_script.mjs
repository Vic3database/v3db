...
assert.match(source, /grep -Fq '\"terrainRuns\":' "\$STAGE\/versions\/1\.13\.9\/map-data\.js"/, "deployment script must verify the current terrain data marker");
...
assert.match(source, /grep -Fq '\"terrainRuns\":' "\$STAGE\/vc\/map-data\.js"/, "deployment script must verify the VC terrain data marker");
assert.match(source, /grep -Fq 'stateTraitFilters' "\$STAGE\/vc\/index\.html"/, "deployment script must verify the VC state trait filter marker");
