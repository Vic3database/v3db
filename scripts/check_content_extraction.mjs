...
const dataRoot = path.join(root, "database", process.argv[2] || "vic3_1.13.9");
const index = JSON.parse(fs.readFileSync(path.join(dataRoot, "content-index.json"), "utf8"));
const expectedVersion = process.argv[3] || "1.13.9";
const expectedEvents = expectedVersion === "1.13.9" ? 2261 : 2264;
const expectedGameEvents = expectedVersion === "1.13.9" ? 2236 : 2239;
const expectedEventSources = expectedVersion === "1.13.9" ? 331 : 328;
const expectedEventDefinitions = expectedVersion === "1.13.9" ? 2261 : 327;
const expectedEventOptions = expectedVersion === "1.13.9" ? 4836 : 4840;

if (index.version !== expectedVersion) throw new Error(`unexpected version: ${index.version}`);
for (const [kind, expected] of [["journal_entries", 419], ["events", expectedEvents], ["decisions", 60], ["journal_entry_groups", 27]]) {
...
if (index.counts.source_files.scanned.events !== expectedEventSources) throw new Error("event subdirectories were not scanned");
...
if (index.counts.journal_entries_game !== 418 || index.counts.events_game !== expectedGameEvents || index.counts.decisions_game !== 60) throw new Error("content classification counts changed");
if (expectedVersion === "1.13.9" && (index.coverage.events.en.title !== 2204 || index.coverage.events.zhHans.title !== 2204)) throw new Error("event localization coverage changed");
const expectedCountryEvents = expectedVersion === "1.13.9" ? 2238 : 2241;
if (index.distributions.event_types.country_event !== expectedCountryEvents || index.distributions.event_types.state_event !== 14) throw new Error("event type distribution changed");
if (index.counts.event_options !== expectedEventOptions) throw new Error("event option count changed");
if (sources.events.length !== expectedEventSources || (expectedVersion === "1.13.9" && sources.events.reduce((total, row) => total + row.definitions, 0) !== expectedEventDefinitions)) throw new Error("source manifest mismatch");
