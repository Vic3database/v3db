(function () {
  ...
  function createCompanySolverModel(companies, targetKeys) {
    ...
    const normalizedRows = (companies || []).map(normalizeCompany).filter((company) => company.key).map((company) => {
      ...
      for (const rawSelectedKeys of company.optionStates) {
        ...
        if (!previous || state.selectedKeys.length < previous.selectedKeys.length || (state.selectedKeys.length === previous.selectedKeys.length && state.selectedKeys.join("|") < previous.selectedKeys.join("|"))) stateByTargetMask.set(coverageMask.toString(), state);
      }
    ...
    const candidatesByTarget = normalizedTargets.map((_key, targetIndexValue) => {
      ...
      const candidates = [];
      states.forEach((companyStates, companyIndex) => companyStates.forEach((state, stateIndex) => {
  ...
  function solveCompanyCombinations(model, options = {}) {
    ...
    const buildSolution = (coveredMask) => {
      const selectedRows = [...selected.entries()].map(([companyIndex, stateIndex]) => ({ companyIndex, company: model.companies[companyIndex], state: model.states[companyIndex][stateIndex] }))
  ...
  window.COMPANY_SOLVER_CORE = { createCompanySolverModel, solveCompanyCombinations, summarizeCompanyUsage };
}());
