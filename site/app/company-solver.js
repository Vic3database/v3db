...
function renderCompanySolverBoard() {
  ...
  if (!companySolverAvailable()) {
    ...
    els.activeHint.textContent = t("board.company.solverUnavailable", "公司产业求解器仅支持原版 1.13.11");
    els.countryList.innerHTML = '<p class="empty">' + escapeHtml(t("board.company.solverUnavailable", "公司产业求解器仅支持原版 1.13.11")) + "</p>";
    return;
  }
  ...
  els.countryList.innerHTML = '<section class="company-solver-shell"><div class="company-solver-toolbar"><div><h2>' + escapeHtml(solverT("board.company.solverTitle", "公司产业求解器")) + '</h2><p>' + escapeHtml(solverT("board.company.solverDescription", "选择希望覆盖的建筑，查找公司组合。")) + '</p></div><div class="company-solver-actions">' + companyCount + restrictionToggle + constructionToggle + '<button type="button" class="company-solver-run" data-company-solver-run ' + (selected.size ? "" : "disabled") + '>' + escapeHtml(solverT("board.company.solverRun", "执行")) + '</button></div></div><div class="company-solver-building-groups">' + groups + '</div>' + renderCompanySolverPrestigeFilters() + '<div class="company-solver-results-head"><span>' + escapeHtml(resultText) + "</span>" + pagination + '</div>' + usage + '<div class="company-solver-layout"><div class="company-solver-card-list">' + solver.solutions.map((solution, index) => renderCompanySolverCard(solution, (solver.page - 1) * COMPANY_SOLVER_PAGE_SIZE + index + 1)).join("") + (solver.status === "complete" && !solver.solutions.length ? '<p class="empty">' + escapeHtml(solverT("board.company.solverNoResult", "没有可覆盖全部建筑的组合。")) + "</p>" : "") + (pagination ? '<div class="company-solver-pagination-bottom">' + pagination + '</div>' : "") + '</div></div></section>';
